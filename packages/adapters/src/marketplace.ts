...
export interface MarketplaceOptions { outDir: string; pluginDir: Omit<Parameters<typeof generateClaudePlugin>[0], "outDir"> }
...
export function renderMarketplace(options: MarketplaceOptions): Record<string, string> {
  ...
  const plugin = renderClaudePlugin({ outDir: join(options.outDir, "plugins", "agent-graph"), ...options.pluginDir });
